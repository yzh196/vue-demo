<template>
  <div class="checkout">
        ...
        <div class="item-list-wrap">
          <div class="cart-item">
            ...
            <ul class="cart-item-list">
              <li :key="item.productId" v-for="item in productList">
                <div class="cart-tab-1">
                  <div class="cart-item-check">
                    <a :class="{'check':item.checked}" @click="changeCheck(item)" class="item-check-btn">
                  ...
                  <div class="item-include">
                    <dl>
                      <dt>赠送:</dt>
                      <dd :key="gift.partsId" v-for="gift in item.parts">{{gift.partsName}}</dd>
                    </dl>
                  </div>
                </div>
                <div class="cart-tab-2">
                  <div class="item-price">{{item.productPrice | filterPrice}}</div>
                </div>
                  ...
                  <div class="item-quantity">
                    <div class="select-self select-self-open">
                      <div class="quantity">
                        ...
                        <input disabled type="text" v-model="item.productQuantity" value="0">
                        <a @click="changeQuantity(item,1)"> + </a>
                      </div>
                    </div>
                ...
                <div class="cart-tab-4">
                  <div class="item-price-total">{{item.productQuantity * item.productPrice | filterPrice}}</div>
                </div>
                <div class="cart-tab-5">
                  <div class="cart-item-operation">
                    <a @click="delProduct(item)" class="item-edit-btn">
        ...
        <div class="cart-foot-wrap">
          <div class="cart-foot-l">
            <div class="item-all-check">
              <a :class="{'check': checkallFlag}" @click="checkall(true)" class="item-check-btn">
                <svg class="icon icon-ok">
                  <use xlink:href="#icon-ok"></use>
                </svg>
              </a>
            ...
            <div class="item-all-del">
              <a @click="checkall(false)" class="item-del-btn">
                <span>取消全选</span>
              </a>
            </div>
          </div>
          <div class="cart-foot-r">
            ...
            <div class="next-btn-wrap">
              <router-link class="btn btn&#45;&#45;red" style="width: 200px" :to="{name: 'address'}">结账</router-link>
            </div>
          </div>
        </div>
      </div>
    ...
    <div :class="{'md-show' : delFlag}" class="md-modal modal-msg md-modal-transition" id="showModal">
      <div class="md-modal-inner">
        <div class="md-top">
          <button @click="delFlag=false" class="md-close">关闭</button>
        </div>
        <div class="md-content">
          ...
          <div class="btn-wrap col-2">
            <button @click="confirmDelProduct" class="btn btn--m" id="btnModalConfirm">Yes</button>
            <button @click="delFlag=false" class="btn btn--m btn--red" id="btnModalCancel">No</button>
          </div>
        </div>
      </div>
    </div>
    <div @click="delFlag=false" class="md-overlay" id="showOverLay" v-show="delFlag"></div>
  </div>
</template>
        ...
        if (state) {
          ...
          this.productList.forEach(function (item) {
            if (typeof item.checked == 'undefined') {
              ...
              //(item)=>{this.$set(item,'checked',true)}
            } else {
              item.checked = true;
            }
          });
